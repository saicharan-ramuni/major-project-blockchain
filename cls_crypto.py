...
class CLSEngine:
    ...
    def import_key_data(self, identity: str, key_data: dict):
        """
        Import key material issued by an external PKG (e.g. Flask server).
        Allows a test client to sign with keys generated by the server's PKG,
        so that Flask's verify() / batch_verify() will accept the signatures.

        key_data must contain: sk {x, d}, pk {X_hex, R_hex}, pseudo_id, h1_val
        """
        with self._lock:
            ...
            self._pseudo_to_real[key_data["pseudo_id"]] = identity
